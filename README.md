...
Для отделения структурных дисбалансов от процентного шума рассчитывается отклонение фактической цены фьючерса ($F_{\text{market}}$) от теоретической оценки Cost-of-Carry ($F_{\text{theory}}$):

$$F_{\text{theory}} = S_{\text{adj}} \times \left(1 + (r - q)\frac{T}{365}\right)$$

$$\text{Abnormal\_Basis} = F_{\text{market}} - F_{\text{theory}}$$

Полученный ряд стандартизируется:
$$Z = \frac{\text{Abnormal\_Basis} - \mu}{\sigma}$$
